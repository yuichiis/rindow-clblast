...
RINDOW_CLBLAST_VERSION=`fgrep "# define PHP_RINDOW_CLBLAST_VERSION" php_rindow_clblast.h | cut -d " " -f 4 | cut -d "\"" -f 2`
. /etc/os-release
OS_VERSION=$ID$VERSION_ID
echo EXTENSION_DIR=$EXTENSION_DIR
echo INI_DIR=$INI_DIR
echo RINDOW_CLBLAST_VERSION=$RINDOW_CLBLAST_VERSION
echo OS_VERSION=$OS_VERSION
rm -rf $PKG_WORK
mkdir -p $PKG_WORK$EXTENSION_DIR
mkdir -p $PKG_WORK$INI_DIR
mkdir -p $PKG_WORK/DEBIAN
cp modules/rindow_clblast.so $PKG_WORK$EXTENSION_DIR/.
chmod 744 $PKG_WORK$EXTENSION_DIR/rindow_clblast.so
cp conf/20-rindow_clblast.ini $PKG_WORK$INI_DIR/.
chmod 744 $PKG_WORK$INI_DIR/20-rindow_clblast.ini
sed -e s/%PHP_RINDOW_CLBLAST_VERSION%/$RINDOW_CLBLAST_VERSION/g debian/control | \
